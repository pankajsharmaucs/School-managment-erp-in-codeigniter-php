...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->request->format && $this->request->body) {
            ...
            $this->request->body = $serviceFormat->factory($this->request->body, $this->request->format)->to_array();
    ...
    public function response($data = NULL, $http_code = NULL, $continue = FALSE)
    {
        ...
        if (!$isProfilingEnabled) {
            ...
            elseif ($data !== NULL) {
                ...
                if (method_exists($serviceFormat, 'to_' . $this->response->format)) {
                    // Set the format header


                    $this->output->set_content_type($this->_supported_formats[$this->response->format], strtolower($this->config->item('charset')));
                    $output = $serviceFormat->factory($data)->{'to_' . $this->response->format}();

                    // An array must be parsed as a string, so as not to cause an array to string error
                    // Json is the most appropriate form for such a data type
                    if ($this->response->format === 'array') {
                        $output = $serviceFormat->factory($output)->{'to_json'}();
                    }
                } else {
                    // If an array or object, then parse as a json, so as to be a 'string'
                    if (is_array($data) || is_object($data)) {
                        $data = $serviceFormat->factory($data)->{'to_json'}();
                    }
